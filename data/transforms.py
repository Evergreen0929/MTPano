...
class ResizeByScale:
    """Resize the input by a fixed scale factor.
    
    This is similar to RandomScaling but uses a deterministic scale factor,
    making it suitable for fixed preprocessing steps like downsampling.

    Args:
      scale_factor (float): The factor by which to scale the image and labels.
                            e.g., 0.5 for half size, 2.0 for double size.
    Returns:
        sample: The input sample resized.
    """
    ...
    def resize(self, key, unscaled):
        """
        Scales the input data using the instance's scale_factor.

        Args:
            key (str): The key from the sample dictionary (e.g., 'image', 'depth').
            unscaled: The numpy array (image or target) to be resized.

        Returns:
            scaled: The resized numpy array.
        """
        # No resizing if scale_factor is 1.0.
        if self.scale_factor == 1.0:
            return unscaled

        image_shape = np.shape(unscaled)[0:2]
        new_dim = tuple([max(16, int(round((x * self.scale_factor) / 16.0) * 16)) for x in image_shape])
...
class RandomHorizontalPanoFlip:
    """Horizontally flip the given image and ground truth randomly."""
...
class RandomPanoRoll:

    def __init__(self, max_roll_frac=1.0):
        self.max_roll_frac = max_roll_frac
        if not (0.0 < self.max_roll_frac <= 1.0):
             raise ValueError('max_roll_frac must be between (0.0, 1.0].')

    def get_roll_factor(self, pano_w):
        roll_frac = random.uniform(-self.max_roll_frac, self.max_roll_frac)
        roll_shift_pix = int(roll_frac * pano_w)
        roll_angle_rad = (roll_shift_pix / pano_w) * 2 * math.pi
        return roll_shift_pix, roll_angle_rad

    def roll(self, data, roll_shift_pix):
        return np.roll(data, roll_shift_pix, axis=1)

    def rotate_normals(self, normals, roll_angle_rad):
        H, W, _ = normals.shape
        flat_normals = normals.reshape(-1, 3) 
        
        # 图像向右平移 roll_shift_pix > 0
        # 相当于将相机姿态向左旋转了 -roll_angle_rad (负 yaw)
        # 因此，世界法线向量需要进行 roll_angle_rad 的旋转，以保持与新姿态的一致性。
        a = roll_angle_rad 
        cos_a = math.cos(a)
        sin_a = math.sin(a)

        # 围绕 Y 轴的旋转矩阵 R_yaw (世界坐标系 N-Space: Y-Up)
        # R = [ cos(a)  0  sin(a) ]
        #     [ 0       1  0      ]
        #     [-sin(a)  0  cos(a) ]
        R_yaw = np.array([
        ...
        rotated_flat_normals = flat_normals @ R_yaw.T
        
        rotated_normals = rotated_flat_normals.reshape(H, W, 3)
        
        return rotated_normals

    def __call__(self, sample):
        pano_w = np.shape(sample['image'])[1]
        roll_shift_pix, roll_angle_rad = self.get_roll_factor(pano_w)
        ...
        for key, val in sample.items():
            ...
            rolled_data = self.roll(val, roll_shift_pix)
            
            if key == 'normals':
                sample[key] = self.rotate_normals(rolled_data, roll_angle_rad)
            else:
                sample[key] = rolled_data
                
        return sample

    def __repr__(self):
        return f"{self.__class__.__name__}(max_roll_frac={self.max_roll_frac})"
