...
class RandomScaling:
    ...
    def __call__(self, sample):
        ...
        for key, val in sample['persp'].items():
            if key == 'meta':
                continue
            sample['persp'][key] = self.scale(key, val, scale=random_scale)
        return sample
...
class PadImage:
    ...
    def __call__(self, sample):
        for key, val in sample['persp'].items():
            if key == 'meta':
                continue
            sample['persp'][key] = self.pad(key, val)
        return sample
...
class RandomCrop:
    ...
    def __call__(self, sample):
        crop_location = self.get_random_crop_loc(sample['persp']['image'])
        if self.cat_max_ratio < 1.:
            # Repeat 10 times
            if 'semseg' in sample['persp'].keys():
                for _ in range(10):
                    seg_tmp = self.random_crop('semseg', sample['persp']['semseg'], crop_location)
        ...
        for key, val in sample['persp'].items():
            if key == 'meta':
                continue
            sample['persp'][key] = self.random_crop(key, val, crop_location)
        return sample
    ...
    def __call__(self, sample):
        if random.random() < self.p:
            for key, val in sample['persp'].items():
                if key == 'meta':
                    continue
                sample['persp'][key] = np.fliplr(val).copy()
                if key == 'normals':
                    sample['persp'][key][:, :, 0] *= -1
        return sample
    ...
    def __call__(self, sample):
        ...
        Returns:
            dict: Normalized results, 'img_norm_cfg' key is added into
                result dict.
        """
        sample['persp']['image'] = self.normalize_img(sample['persp']['image'])
        sample['pano']['image'] = self.normalize_img(sample['pano']['image'])
        return sample
...
class ToTensor:
    ...
    def __call__(self, sample):
        for key, val in sample['persp'].items():
            if key == 'meta':
                continue
            sample['persp'][key] = torch.from_numpy(val.transpose((2, 0, 1))).float()
            sample['pano'][key] = torch.from_numpy(val.transpose((2, 0, 1))).float()
        return sample
...
class AddIgnoreRegions:
    ...
    def __call__(self, sample):
        for elem in sample['persp'].keys():
            tmp = sample['persp'][elem]
            if elem == 'normals':
                # Check areas with norm 0
                norm = np.sqrt(tmp[:, :, 0] ** 2 +
                               tmp[:, :, 1] ** 2 + tmp[:, :, 2] ** 2)
                tmp[norm == 0, :] = 255
                sample['persp'][elem] = tmp
            elif elem == 'depth':
                tmp[tmp == 0] = 255
                sample['persp'][elem] = tmp

        for elem in sample['pano'].keys():
            tmp = sample['pano'][elem]
            if elem == 'normals':
                # Check areas with norm 0
                norm = np.sqrt(tmp[:, :, 0] ** 2 +
                               tmp[:, :, 1] ** 2 + tmp[:, :, 2] ** 2)
                tmp[norm == 0, :] = 255
                sample['pano'][elem] = tmp
            elif elem == 'depth':
                tmp[tmp == 0] = 255
                sample['pano'][elem] = tmp
        return sample
...
class PhotoMetricDistortion:
    ...
    def __call__(self, sample):
        ...
        sample['persp']['image'] = img.astype(np.float32)
        return sample
