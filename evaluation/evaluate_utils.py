...
def get_single_task_meter(p, database, task):
    ...
    if task == 'semseg':
        ...
        if database == 'Matterport3D':
            return NormalsMeter(ignore_index=p.ignore_index, align_360mtl=True)
        else:
        ...
        return DepthMeter(database, ignore_index=p.ignore_index, max_depth=p['depth_max_val']) 
