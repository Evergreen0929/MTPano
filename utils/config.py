...
def parse_task_dictionary(db_name, task_dictionary):
    ...
    if 'include_semseg' in task_dictionary.keys() and task_dictionary['include_semseg']:
        ...
        if db_name == 'PASCALContext':
            ...
            task_cfg.NUM_OUTPUT[tmp] = 22
        elif db_name == 'PanoMTDU':
            task_cfg.NUM_OUTPUT[tmp] = 150
        else:
    ...
    if 'include_depth' in task_dictionary.keys() and task_dictionary['include_depth']:
        ...
        if db_name in ['Stanford2D3D', 'Matterport3D']:
            other_args['depth_max_val'] = 10.0
        else:
            other_args['depth_max_val'] = None
    ...
    if 'include_normals' in task_dictionary.keys() and task_dictionary['include_normals']:
        # Surface Normals 
        tmp = 'normals'
        assert(db_name in ['PASCALContext', 'NYUD', 'Structured3D', 'Stanford2D3D', 'Matterport3D', 'PanoMTDU'])
        task_cfg.NAMES.append(tmp)
...
def create_config(exp_file, params):
    ...
    elif cfg['train_db_name'] in ['Structured3D', 'Stanford2D3D']:
        cfg.TRAIN = edict()
        cfg.TEST = edict()
        cfg.TRAIN.SCALE = (448, 576)
        cfg.TEST.SCALE = (448, 576)
        cfg.TRAIN.PERSP_SCALE = (448, 576)
        cfg.TEST.PERSP_SCALE = (448, 576)
        cfg.TRAIN.PANO_SCALE = (512, 1024)
        cfg.TEST.PANO_SCALE = (512, 1024)

    elif cfg['train_db_name'] in ['PanoMTDU', 'Matterport3D', 'SynPASS', 'Deep360']:
        cfg.TRAIN = edict()
        cfg.TEST = edict()
